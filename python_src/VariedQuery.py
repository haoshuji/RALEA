...
def VariedQuery(options,X_test,Y_test):			
	...
	for i in range(options['num_queries']):
		...
		for alg_name in options['algorithm_names']:
			...
			_results_one_query[alg_name]['acc'] = []
		...
		for k in range(options['num_folds']):
			...
			for alg_name in options['algorithm_names']:
				...
				############################################################################
				if 'EWAF' in alg_name :
					que, reg, time, acc = EWAF(XTest,YTest,forecaster_options)							
				else:
					que, reg, time, acc = GF(XTest,YTest,forecaster_options)
				############################################################################
				############################################################################
				print('{}: {}, {}, {}, {}'.format(alg_name, que, reg, acc, time))
				...
				_results_one_query[alg_name]['acc'].append(acc)			
		...
		for alg_name in options['algorithm_names']:
			...
			if alg_name in ['GF','EWAF']	and i > 0:
				...
				results_all_query[alg_name]['acc'].append(results_all_query[alg_name]['acc'][0])
				results_all_query[alg_name]['stdAcc'].append(results_all_query[alg_name]['stdAcc'][0])
			else:	
				...
				results_all_query[alg_name]['acc'].append(round(np.mean(_results_one_query[alg_name]['acc']),3))
				results_all_query[alg_name]['stdAcc'].append(round(np.std(_results_one_query[alg_name]['acc']),3))
	print '\n'	
